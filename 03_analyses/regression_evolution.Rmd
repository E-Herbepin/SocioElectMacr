...
modele <- PresF %>%
  ...
  lm(diff_pourc_macron ~ pourc_macron_17 + libdens + cs_ouvr_pourc + cs_cpis_pourc + cs_pi_pourc + cs_agri_pourc + cs_acce_pourc + Age_q30_40_pourc + Age_q40_50_pourc + Age_q50_60_pourc + Age_q60_70_pourc + Age_q70_100_pourc + log_logoccas_pourc + log_ressecond_pourc + log_vacants_pourc + prop_h, data = .)
summary(modele)

# ajouter genre + logement + centrer
...
modele_td <- tidy(modele2) |> # Récupère un tableau propre
  select(term, estimate) |>
  mutate(
    term = recode(term,
      `(Intercept)` = "Constante",
...
modele_td |>
  ...
  gtsave("regression_evolution_rural.docx", path = "../03_analyses/")
...
PresF$libdens_group_dec <- ifelse(PresF$libdens %in% c("Ceintures urbaines", "Petites villes", "Grands centres urbains", "Centres urbains intermédiaires"),"Urbain", PresF$libdens)
...
model_mult <- lmer(diff_pourc_macron ~ pourc_macron_17 + cs_ouvr_pourc + cs_cpis_pourc + cs_pi_pourc + cs_agri_pourc + cs_acce_pourc + Age_q30_40_pourc + Age_q40_50_pourc + Age_q50_60_pourc + Age_q60_70_pourc + Age_q70_100_pourc + log_logoccas_pourc + log_ressecond_pourc + log_vacants_pourc + prop_h +
