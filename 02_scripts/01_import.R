...
if (!file.exists("01_data/Présidentielles 2017-2022/Présidentielles&Recensement.csv")) {
  ...
  if (!file.exists("01_data/Présidentielles 2017-2022/Pres17T1_DL.xls")) {
    #Si le fichier n'est pas trouvé
    download.file(
      #On le télécharge
      url = paste0(
        "https://www.data.gouv.fr/fr/datasets/r/77ed6b2f-c48f-4037-8479-50af74fa5c7a"
      ),
  ...
  if (!dir.exists("01_data/Communes_nouvelles")) {
    ...
    for (x in 2017:2022) {
      download.file(
        url = paste0(
          "https://www.insee.fr/fr/statistiques/fichier/2549968/Communes_nouvelles_",
          x ,
          if_else(x <= 2019, ".xls", ".xlsx")
        ),
        destfile = paste0(
          "01_data/Communes_nouvelles/Communes_nouvelles_",
          x,
          if_else(x <= 2019, ".xls", ".xlsx")
        ),
        mode = "wb"
  ...
  if (!file.exists("01_data/Présidentielles 2017-2022/Pres17T1_recodé.csv")) {
    Pres17T1 <- read_xls(here("01_data/Présidentielles 2017-2022", "Pres17T1_DL.xls"),
                         skip = 3)
    
    for (i in c(19 + c(0:10) * 7)) {
      #Pour chaque "panneau" (i= le numéro de la colonne)
      for (x in 1:nrow(Pres17T1)) {
        #Pour chaque commune
        Pres17T1[x, paste0("Nb_17_Voix_", str_to_title(as.character(Pres17T1[x, i +
                                                                               2])))] <- Pres17T1[x, i + 4] #la case de la commune dans la colonne "NomDuCandidat_Voix" prend la valeur de la colonne "Voix" qui suit le panneau qu'on regarde
      }
    }
    
    Pres17T1 %<>% select(-c(19:95, which(
      grepl("%", names(Pres17T1)) |
        grepl("[.][.]", names(Pres17T1))
    ))) %>% # On enlève les colonnes que l'on a utilisées pour créer les colonnes "NomDuCandidat_Voix" etc. ET les colonnes recalculables (pourcentages)
      rename(
        "Libellé_commune" = "Libellé de la commune",
    ...
    Pres17T1 %<>% filter(!str_detect(
      Pres17T1$Libellé_département,
    ...
    Pres17T1$Code_département <- case_when(
      ...
      Pres17T1$Libellé_département == "Guadeloupe" ~ "971",
      # Or la Corse, ces départements ne sont pas dans les résutats 2022
      ...
      Pres17T1$Libellé_commune == "Saint-Martin" &
        Pres17T1$Libellé_département == "Saint-Martin/Saint-Barthélemy" ~ "977",
    ...
    Pres17T1 %<>%
      mutate(
        ...
        Code_département = str_pad(
          as.character(Code_département),
          width = 2,
          pad = "0"
        ),
        DepCom = str_pad(
          paste0(as.character(Code_département), Code_commune),
          width = 3,
          pad = "0"
        )
      )
    ...
    write.csv(Pres17T1,
              "01_data/Présidentielles 2017-2022/Pres17T1_recodé.csv",
              row.names = FALSE) # On exporte en CSV (plus pratique pour la suite)
  } else {
    Pres17T1 <- read.csv("01_data/Présidentielles 2017-2022/Pres17T1_recodé.csv")
  }
  ...
  if (!file.exists("01_data/Présidentielles 2017-2022/Pres22T1_recodé.csv")) {
    Pres22T1 <- read_delim(
      here("01_data/Présidentielles 2017-2022", "Pres22T1_DL.csv"),
      delim = ",",
      col_select = -c(1)
    )
    ...
    Pres22T1 %<>% select(-c(1, 5, 4, 24, 26, 27, "num_tour", which(
      grepl("pourc", names(Pres22T1)) |
        grepl("rapport", names(Pres22T1))
    ))) %>%
    ...
    Pres22T1 %<>%
      mutate(
        #Cas de la Corse
        Code_département = case_when(
          Libellé_département == "Corse-du-Sud" ~ "2A",
          Libellé_département == "Haute-Corse" ~ "2B",
          TRUE ~ as.character(Code_département)
        ),
        #On normalise l'ecriture du code commune
        Code_département = str_pad(Code_département, width = 2, pad = "0"),
        
        #On ajoute un identifiant unique pour les communes
        DepCom = str_pad(
          paste0(if_else(
            #Si le code département est 971, 972, 973 ou 974
            str_detect(Code_département, "^97"),
            #On le remplace par 97 et on lui ajoute le code commune
            "97",
            #Sinon, on le remplace par le code département normal et on lui ajoute le code commune
            as.character(Code_département)
          ), Code_commune),
          width = 5,
          pad = "0"
    ...
    write.csv(Pres22T1,
              "01_data/Présidentielles 2017-2022/Pres22T1_recodé.csv",
              row.names = FALSE) # On exporte en CSV (plus pratique pour la suite)
  } else {
    Pres22T1 <- read.csv("01_data/Présidentielles 2017-2022/Pres22T1_recodé.csv")
  }
  ...
  if (!file.exists("01_data/Communes_nouvelles/Communes_nouvelles_fusionnées.csv")) {
    ...
    Communes_nouvelles <- bind_rows(
      ...
      filter(Date >= Date_elections_Début &
               Date <= Date_elections_Fin)
    
    Communes_nouvelles <- rbind(
      Communes_nouvelles,
      data.frame(
    ...
    Communes_nouvelles <- Communes_nouvelles %>%
      merge(
        #on ajoute la colonne des noms de 2022 en fonction du Code
      ...
      mutate(
        ...
        NomCN = if_else(
          NomCN != Libellé_commune &
            #Que les noms entre 2022 et communes nouvelles sont différents
            NomCN_tr == Libellé_commune_tr,
          Libellé_commune,
          NomCN
        ),
        # On modifie les libellés mais aussi les différents codes
        
        DepComN = str_pad(as.character(DepComN), width = 5, pad = "0")
    ...
    write.csv(
      Communes_nouvelles,
      "01_data/Communes_nouvelles/Communes_nouvelles_fusionnées.csv"
    )
  } else {
    Communes_nouvelles <- read.csv("01_data/Communes_nouvelles/Communes_nouvelles_fusionnées.csv")
  }
  ...
  if (!file.exists("01_data/Présidentielles 2017-2022/Présidentielles_fusionnées.csv")) {
    ...
    Pres17T1_fusion <- Pres17T1 %>%
      ...
      mutate(
        Libellé_commune = if_else(!is.na(NomCN), NomCN, Libellé_commune),
        # On modifie les libellés mais aussi les différents codes
        ...
        DepCom = str_pad(
          if_else(!is.na(NomCN), as.character(DepComN), DepCom),
          width = 5,
          pad = "0"
        )
      )
    ...
    Pres17T1_fusion <- Pres17T1_fusion %>%
      left_join(#on ajoute la colonne des noms de 2022 en fonction du Code
        Pres22T1[, c("DepCom", "Libellé_commune")], suffix = c("", "_22"), by = "DepCom") %>%
      mutate(
        ...
        Libellé_commune = case_when(
          Libellé_commune_17_tr == Libellé_commune_22_tr #Si les noms nettoyés coincident
          &
            Libellé_commune_22 != Libellé_commune #Et qu'ils sont "différents" à la base
          &
            !is.na(Libellé_commune_22) ~ Libellé_commune_22,
          #On remplace par le nom 2022
          Dist %in% c(1:3, 5, 7) ~ Libellé_commune_22,
          #S'il y entre 1 et 3 caractères de différents (plus deux avec 5 et 7)
          DepCom %in% c(
            "06128",
            "08165",
            "12133",
          ...
          TRUE ~ Libellé_commune
        ),
        #Sinon, on garde le nom de base
        
        Libellé_département = case_when(
          Libellé_commune == "Vallons-de-l'Erdre" ~ "Loire-Atlantique",
    ...
    #A priori inutile, mais on s'assure que le code soit dans le bon format
    Pres$DepCom %<>% str_pad(width = 5, pad = "0")
    
    write.csv(
      Pres,
      "01_data/Présidentielles 2017-2022/Présidentielles_fusionnées.csv",
      row.names = FALSE
    )
  } else {
    Pres <- read.csv("01_data/Présidentielles 2017-2022/Présidentielles_fusionnées.csv")
  }
  ...
  census20_act2a <- read_xlsx(here("01_data/census20", "TD_ACT2A_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
        CODGEO %in% c("14712", "14666") ~ "14712",
        CODGEO == "27676" ~ "27058",
        TRUE ~ CODGEO
      )
    ) %>%
  ...
  census20_act2b <- read_xlsx(here("01_data/census20", "TD_ACT2B_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_act3 <- read_xlsx(here("01_data/census20", "TD_ACT3_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_act4 <- read_xlsx(here("01_data/census20", "TD_ACT4_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_emp1 <- read_xlsx(here("01_data/census20", "TD_EMP1_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_emp3 <- read_xlsx(here("01_data/census20", "TD_EMP3_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_emp4 <- read_xlsx(here("01_data/census20", "TD_EMP4_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_for2 <- read_xlsx(here("01_data/census20", "TD_FOR2_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_log1 <- read_xlsx(here("01_data/census20", "TD_LOG1_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_img1A <- read_xlsx(here("01_data/census20", "TD_IMG1A_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_nat1 <- read_xlsx(here("01_data/census20", "TD_NAT1_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  census20_pop1B <- read_xlsx(here("01_data/census20", "TD_POP1B_2020.xlsx"), skip = 10) %>%
    mutate(
      LIBGEO = if_else(CODGEO %in% c("14712", "14666"), "Saline", LIBGEO),
      # On modifie les libellés mais aussi les différents codes
      CODGEO = case_when(
  ...
  densite_communes <- read_xlsx(here(
    "01_data/densite_communes",
  ...
  communes %<>% mutate(DepCom = case_when(
  ...
  write.csv(
    PresF,
    "01_data/Présidentielles 2017-2022/Présidentielles&Recensement.csv",
    row.names = FALSE
  )
} else {
  PresF <- read.csv("01_data/Présidentielles 2017-2022/Présidentielles&Recensement.csv")
}

# DONNEES GEOLOC ----

shp_path <- here("01_data/geoloc", "fr-esr-referentiel-geographique.shp")
